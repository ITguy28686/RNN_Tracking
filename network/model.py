...
class Model:
    def __init__(self, mat_x, det_anno, prev_asscoia, h_state_init, is_training, keep_prob, data_format='NCHW'):
        ...
        self.coord_flow, self.association_flow, self.rnn_state = self.mynet(self.mat_x, self.det_anno, self.prev_asscoia, self.h_state_init, data_format, keep_prob)
    ...
    def mynet(self, mat_x, det_anno, prev_asscoia, h_state_init_1, h_state_init_2, data_format='NCHW', keep_prob=0.5) :
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            with tf.variable_scope("coord_gru"):
                ...
                tensor_flow, rnn_coord_state = self._gru_layer(input = tensor_flow, num_units = self.GRU_SIZE, h_state_init = h_state_init_1, scope='GRU_coord_epsilon')
            ...
            epsilon_flow = slim.fully_connected(tensor_flow, record_N, scope='epsilon_final', activation_fn=None)
            
            
            with tf.variable_scope("associa_gru"):
                
                associa_input = tf.reshape(associa_input, ( -1, 1, associa_input.get_shape()[1]))
                associa_input = tf.tile(associa_input, [1, record_N, 1])
                ...
                associa_flow = slim.fully_connected(associa_flow, self.GRU_SIZE, scope='fc_associa_gru')
